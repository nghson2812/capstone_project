    ...
    for server in servers:
        ...
        def client_test(host, server_ip, group, srv_name):
            for i in range(num_requests):
                ...
                bw = parse_bandwidth(output)
                bandwidth_data[group][srv_name].append(bw)
                print(f'{host.name} -> {srv_name} [UDP Request {i+1}]: {bw:.2f} Mbits/sec')
    ...
    for server in servers:
        ...
        for host in all_hosts:
            ...
            print(f'{host.name} -> {server_name} [UDP]: {result["bandwidth_mbps"]:.2f} Mbps, '
                  ...
                  f'in {result["duration_sec"]:.2f} sec')
...
def custom_cli(net):
    while True:
        ...
        if cmd == "exit":
            break
        elif cmd == "run_iperf_tcp":
            run_iperf_tcp(net)
        elif cmd == "run_iperf_udp":
            run_iperf_udp(net)
        elif cmd == "run_iperf_tcp_mt":
            run_iperf_multithreaded_tcp(net, num_requests=3)
        elif cmd == "run_iperf_udp_mt":
            run_iperf_multithreaded_udp(net, num_requests=3)
        elif cmd == "h1s ping h1f":
            ping_h1s_to_h1f(net)
        elif cmd == "pingall":
            ping_all(net)
            ...
            print("Unknown command. Available: run_iperf_tcp, run_iperf_udp, run_iperf_tcp_mt, run_iperf_udp_mt, pingall, exit, sh <host_cmd>")
