...
def run_iperf(net):
    ...
    # Start iperf servers on all 3 servers
    info('*** Starting iperf servers on srv1, srv2, srv3\n')
    for server in servers:
        server.cmd('killall -9 iperf')
        server.cmd('iperf -s &')

    time.sleep(2)  # <-- Add this delay to allow servers to fully start

    # Run iperf client from each host to each server
    for host in all_hosts:
        group = 'student' if host.name.endswith('s') else 'faculty'
        for server in servers:
            info(f'*** {host.name} testing connection to {server.name} ({server.IP()})\n')
            output = host.cmd(f'iperf -c {server.IP()} -t 2')
            bw = parse_bandwidth(output)
            bandwidth_data[group][server.name].append(bw)
            print(f'{host.name} -> {server.name}: {bw:.2f} Mbits/sec')

    # Compute and print averages
    print("\n=== Average Bandwidth (Mbits/sec) ===")
