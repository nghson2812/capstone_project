...
def run_iperf(net):
    ...
    for server in servers:
        ...
        info(f'\n*** Starting iperf server on {server_name}\n')
        ...
        for host in all_hosts:
            ...
            info(f'*** {host.name} testing connection to {server_name} ({server.IP()})\n')
            output = host.cmd(f'iperf -c {server.IP()} -t 2')
            bw = parse_bandwidth(output)
            bandwidth_data[group][server_name].append(bw)
            print(f'{host.name} -> {server_name}: {bw:.2f} Mbits/sec')

        # Stop iperf server on current server
        server.cmd('killall -9 iperf')
        info(f'*** Stopped iperf server on {server_name}\n')
